...
{
    int result = lc3_asm_run(input_filename);
    if (result != 0)
    {
...
int run_lc3_vm(const char *program_filename)
{
    lc3_vm_t vm;

    lc3_vm_init(&vm);

    if (!lc3_vm_load_program(&vm, program_filename))
    {
...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 3 && strcmp(argv[1], "-r") == 0)
    {
        ...
        char *obj_filename = lc3_asm_get_output_filename(argv[2]);

        printf("Running '%s'...\n", obj_filename);
        return run_lc3_vm(obj_filename);
    }
    // lc3_vm <program.obj> to run a pre-assembled program
    else if (argc == 2)
    {
        // VM only mode: <program.obj>
        return run_lc3_vm(argv[1]);
    }
